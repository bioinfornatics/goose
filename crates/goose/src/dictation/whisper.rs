...
/// Maps an ISO 639-1 language code to the corresponding Whisper vocabulary token ID.
/// Token IDs are identical across Whisper model sizes (tiny/base/small/medium/large).
/// Returns `None` for unrecognised language codes.
pub fn whisper_language_token(iso: &str) -> Option<u32> {
    // Token IDs from the Whisper tokenizer (same across tiny/base/small/medium/large).
    const LANGS: &[(&str, u32)] = &[
        ("en", 50259),
        ("zh", 50260),
        ("de", 50261),
...
impl WhisperTranscriber {
    ...
    /// Override the language conditioning token.  Call before `transcribe()`.
    pub fn set_language_token(&mut self, token: u32) {
